...
export const restUrl = {
    ...
    OBJECT_CODE_LIST: '/table/all?objectCode=meta_object&fs=code&s=1000',  // 获取所有元对象code
    OBJECT_CODE_LIST_AS_KV: '/table/all?objectCode=meta_object&fs=code,cn&code->key=&code->value=&cn->label=',

    FIELD_CODE_LIST_BY_OBJECT: '/table/all?objectCode=meta_field&object_code={objectCode}&fs=field_code,en,cn&en->key=&field_code->value=&cn->label=', // 查询某个元对象的元字段的field_code和cn，并以key,value返回
    FIELD_CODE_CONF: '/table/all?objectCode=meta_field&object_code={objectCode}&field_code={fieldCode}&fs=config,id', // 获取某个元字段的逻辑配置
    COMPONENT_CODE_LIST: '/component/list?view={view}', // 获取所有组件列表, 返回数据格式KV
    INSTANCE_CODE_LIST: '/table/all?objectCode=meta_component_instance&fs=code&code->value=', // instanceCode
